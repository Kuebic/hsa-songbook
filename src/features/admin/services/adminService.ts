...
// QueryBuilder implementation for getUsers
async function getUsersWithQueryBuilder(filter?: UserFilter): Promise<{ users: UserWithRole[]; total: number }> {
  const cacheKey = `users:${JSON.stringify(filter || {})}`
  const cached = getCachedResult<{ users: UserWithRole[]; total: number }>(cacheKey)
  if (cached) return cached

  // Build query
  const query = createQueryBuilder(supabase, 'users')
    .select('*', { count: 'exact' })

  // Apply search filter
  if (filter?.search) {
    query.or(`email.ilike.%${filter.search}%,full_name.ilike.%${filter.search}%`)
  }

  // Sorting
  const sortColumn = filter?.sortBy === 'last_sign_in' ? 'updated_at' : filter?.sortBy || 'created_at'
  query.orderBy(sortColumn, { ascending: filter?.sortOrder !== 'desc' })
  ...
  if (userIds.length > 0) {
    const roleQuery = createQueryBuilder(supabase, 'user_roles')
      .select('*')
      .in('user_id', userIds)
      .eq('is_active', true)
    
    const roleResult = await roleQuery.execute()
    if (roleResult.error) {
      console.warn('Error fetching roles:', roleResult.error)
    } else {
      roleData = (roleResult.data as UserRoleRow[]) || []
    }
  }
  ...
  const response = { users, total: count }
  setCachedResult(cacheKey, response)
  return response
}

// Legacy implementation for getUsers
async function getUsersLegacy(filter?: UserFilter): Promise<{ users: UserWithRole[]; total: number }> {
  const cacheKey = `users:${JSON.stringify(filter || {})}`
  const cached = getCachedResult<{ users: UserWithRole[]; total: number }>(cacheKey)
  if (cached) return cached
...
// QueryBuilder implementation for assignRole
async function assignRoleWithQueryBuilder(assignment: RoleAssignment): Promise<void> {
  clearCache()
  ...
  const roleResult = await roleQuery.execute()
  if (roleResult.error) throw new APIError(roleResult.error.message, 400)

  // Log to audit
  const auditQuery = createQueryBuilder(supabase, 'role_audit_log')
    .insert({
      user_id: assignment.userId,
      role: assignment.role,
      action: 'grant',
  ...
  const auditResult = await auditQuery.execute()
...
// Legacy implementation for assignRole
async function assignRoleLegacy(assignment: RoleAssignment): Promise<void> {
  clearCache()
...
// QueryBuilder implementation for revokeRole
async function revokeRoleWithQueryBuilder(userId: string, reason?: string): Promise<void> {
  clearCache()
  ...
  const roleResult = await roleQuery.execute()
  if (roleResult.error) throw new APIError('User role not found', 404)
  
  const currentRole = roleResult.data as { role: string }
  if (!currentRole) throw new APIError('User role not found', 404)

  // Update role to 'user' and deactivate
  const updateQuery = createQueryBuilder(supabase, 'user_roles')
    .update({
      role: 'user',
      is_active: false,
      granted_by: user.id,
  ...
  if (updateResult.error) throw new APIError(updateResult.error.message, 400)

  // Log to audit
  const auditQuery = createQueryBuilder(supabase, 'role_audit_log')
    .insert({
      user_id: userId,
      role: currentRole.role as UserRole,
      action: 'revoke',
      performed_by: user.id,
      reason,
      metadata: {}
...
// Legacy implementation for revokeRole
async function revokeRoleLegacy(userId: string, reason?: string): Promise<void> {
  clearCache()
...
// QueryBuilder implementation for getAuditLog
async function getAuditLogWithQueryBuilder(userId?: string): Promise<AuditLogEntry[]> {
  const cacheKey = `audit:${userId || 'all'}`
  const cached = getCachedResult<AuditLogEntry[]>(cacheKey)
  if (cached) return cached

  // Get the audit log data
  const query = createQueryBuilder(supabase, 'role_audit_log')
    .select('*')
    .orderBy('performed_at', { ascending: false })
    .limit(100)
  ...
  if (userIds.length > 0) {
    try {
      const userQuery = createQueryBuilder(supabase, 'users')
...
// Legacy implementation for getAuditLog
async function getAuditLogLegacy(userId?: string): Promise<AuditLogEntry[]> {
  const cacheKey = `audit:${userId || 'all'}`
  const cached = getCachedResult<AuditLogEntry[]>(cacheKey)
  if (cached) return cached
...
// QueryBuilder implementation for getAdminStats
async function getAdminStatsWithQueryBuilder(): Promise<AdminStats> {
  ...
  let totalUsers = 0
  try {
    const userCountResult = await createQueryBuilder(supabase, 'users')
      .select('*', { count: 'exact' })
      .count()
    
    if (userCountResult.error) {
      console.warn('Error fetching user count:', userCountResult.error)
    } else {
  ...
  let recentChanges = 0
  try {
    const auditCountResult = await createQueryBuilder(supabase, 'role_audit_log')
      .select('*', { count: 'exact' })
      .gte('performed_at', sevenDaysAgo.toISOString())
      .count()
    
    if (auditCountResult.error) {
      console.warn('Error fetching recent changes:', auditCountResult.error)
    } else {
      recentChanges = auditCountResult.count || 0
    }
  } catch (error) {
  ...
  const stats: AdminStats = {
    ...
    regularUserCount: Math.max(0, totalUsers - (roleCountMap.get('admin') || 0) - (roleCountMap.get('moderator') || 0)),
...
// Legacy implementation for getAdminStats
async function getAdminStatsLegacy(): Promise<AdminStats> {
  const cached = getCachedResult<AdminStats>('admin-stats')
  if (cached) return cached
...
export const adminService = {
  async getUsers(filter?: UserFilter): Promise<{ users: UserWithRole[]; total: number }> {
    try {
      return await withMigration(
        'admin.getUsers',
        () => getUsersLegacy(filter),
        () => getUsersWithQueryBuilder(filter)
      )
    } catch (error) {
  ...
  async assignRole(assignment: RoleAssignment): Promise<void> {
    try {
      return await withMigration(
        'admin.assignRole',
        () => assignRoleLegacy(assignment),
        () => assignRoleWithQueryBuilder(assignment)
      )
    } catch (error) {
  ...
  async revokeRole(userId: string, reason?: string): Promise<void> {
    try {
      return await withMigration(
        'admin.revokeRole',
        () => revokeRoleLegacy(userId, reason),
        () => revokeRoleWithQueryBuilder(userId, reason)
      )
    } catch (error) {
  ...
  async getAuditLog(userId?: string): Promise<AuditLogEntry[]> {
    try {
      return await withMigration(
        'admin.getAuditLog',
        () => getAuditLogLegacy(userId),
        () => getAuditLogWithQueryBuilder(userId)
      )
    } catch (error) {
